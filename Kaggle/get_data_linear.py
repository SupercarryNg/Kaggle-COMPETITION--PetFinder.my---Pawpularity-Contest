...
class MyData(Dataset):
    def __init__(self, root_dir, sub_dir):
        ...
        tmp = pd.read_csv(self.csv_path)[['Id', 'Pawpularity']]
        tmp['Pawpularity'] = (tmp['Pawpularity'] - tmp['Pawpularity'].min()) / (
                    tmp['Pawpularity'].max() - tmp['Pawpularity'].min())
        self.popularity = tmp
    ...
    def __getitem__(self, idx):
        ...
        pawpularity = np.float32(self.popularity.loc[self.popularity.Id == id, 'Pawpularity'])
